...
function createLetters(text, rad, offset, frontColor, direction, id) {
  ...
  letters.forEach((letter, index) => {
    ...
    letterElement.style.transform = `translate(-50%, -50%) translate(${x}px, ${y}px) rotateX(90deg) rotateY(${
      angle + 90
    }deg) rotateZ(180deg) translateY(${offset}px)`;
...
function updateLetters(container, totalLetters, op1, op2, colorFront, dir, id) {
  ...
  const rotation =
    ((elapsedTime % animationDuration) / animationDuration) * 360;
  ...
  letters.forEach((letter, index) => {
    ...
    let isActive =
      angleDifference < activeAngleRange / 2 ||
      angleDifference > 360 - activeAngleRange / 2;
...
CTRL.add(CONFIG, "refOpacity", 0, 1, 0.01)
